...
pub fn sub_sort<T, F>(x: &mut [T], forward: bool, comparator: &F)
where
    T: Send,
    F: Sync + Fn(&T, &T) -> Ordering,
{
    if x.len() > 1 {
        ...
        if mid_point >= PARALLEL_THRESHOLD {
            rayon::join(
                || sub_sort(first, true, comparator),
                || sub_sort(second, false, comparator),
            );
        } else {
            sub_sort(first, true, comparator);
            sub_sort(second, false, comparator);
    ...
    F: Sync + Fn(&T, &T) -> Ordering,
{
    if x.len().is_power_of_two() {
        ...
        Err(format!(
            "The length of x is not a power of tow. (x.len(): {})",
            x.len()
        ))
    }
}
...
mod tests {
    ...
    use crate::utils::{is_sorted_ascending, is_sorted_discending, new_u32_vec};
    use crate::SortOrder::*;

    #[test]
    fn sort_to_fail() {
        let mut x = vec![10, 30, 11]; // x.len()が2の冪乗ではない。
    ...
    #[test]
    fn sort_students_name_ascending() {
        let taro = Student::new("Taro", "Yamada", 16);
        let hanako = Student::new("Hanako", "Yamada", 14);
        let kyoko = Student::new("Kyoko", "Ito", 15);
        let ryosuke = Student::new("Ryosuke", "Hayashi", 17);
